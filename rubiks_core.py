...
# ---- Method 5 : SVM (Support Vector Machine) from LUIAN_Part -----------

def extract_features_svm(img_block):
    """
    Extract features as expected by the LUIAN_Part SVM model:
...
def classify_color_svm(img_block):
    ...
    if _svm_clf is None:
        ...
        if not os.path.exists(model_path):
            ...
            model_path = os.path.join(base_dir, "AI_LUIAN_Part", "AI_LUIAN_Part", "svm_color_model.pkl")
